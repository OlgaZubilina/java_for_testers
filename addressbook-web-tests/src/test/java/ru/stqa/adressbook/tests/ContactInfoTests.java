...
public class ContactInfoTests extends TestBase{
    ...
    @Test
    void testEmails(){Allure.step("Checking precondition",step ->{if (app.contacts().getList().size() == 0) {//создание нового контакта  при отсутствии
        ...
        Allure.step("Validating result", step ->{Assertions.assertEquals(expected,emails);});

    }

    @Test
    void testPhones(){Allure.step("Checking precondition",step ->{if (app.contacts().getList().size() == 0) {//создание нового контакта  при отсутствии
        var contact = new ContactData()
                .withFirstname(CommonFunctions.randomString(6))
                .withLastname(CommonFunctions.randomString(6))
                .withAdress(CommonFunctions.randomString(6))
        ...
        var expected = Stream.of(contact.home(),contact.mobile(),contact.work())
                .filter(s->s!=null && ! "".equals(s))
                .collect(Collectors.joining("\n"));
        Allure.step("Validating result",step ->{Assertions.assertEquals(expected,phones);});

    }


    @Test
    void testInfo(){Allure.step("Checking precondition",step ->{if (app.contacts().getList().size() == 0) {//создание нового контакта  при отсутствии
        var contact = new ContactData()
                .withFirstname(CommonFunctions.randomString(6))
                .withLastname(CommonFunctions.randomString(6))
                .withAdress(CommonFunctions.randomString(6))
        ...
        Allure.step("Validating result",step ->{Assertions.assertEquals(expected,contactInfo);});
